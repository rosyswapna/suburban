<?php
$path_to_root = "..";
$page_security = 'SA_VEHICLEINVOICE';
...
if(isset($_GET['TripInvoice'])){
	...
	$vouchers = get_vehicle_vouchers_with_invoice($tripInvoice);
	$_POST['tripinvoice'] = $tripInvoice;

	if(!$vouchers) {
		meta_forward($_SERVER['PHP_SELF'], "ProcessComplete=".$tripInvoice);
	}
}

elseif (isset($_GET['ProcessComplete'])) {

	
   	display_notification_centered( _("Trip Invoice Processed"));

	display_note(print_document_link($_GET['ProcessComplete']."-". ST_SALESINVOICE, _("&Print Trip Invoice"), true, ST_SALESINVOICE));
...
if (isset($_POST['Commit']))
{
	...
	$item = get_item(VEHICLE_COMMISSION);//get trip driver commission in fa as item record
	$location = 'DEF';
...
display_heading(_("Vehicle Trip Details"));
	div_start('items_table');
    	start_table(TABLESTYLE, "width=80%");

	   	$th = array(_("Vehicle Owner"), _("Trip Ids"), _("No Of Days"), _("Amount"));
	...
	if(count($vouchers) > 0) 
	{
		...
		//submit_center_first('Commit', _("Process Vehicle Invoice"), '', 'default');
		submit_center_first('Commit', _("Process Vehicle Invoice"));
